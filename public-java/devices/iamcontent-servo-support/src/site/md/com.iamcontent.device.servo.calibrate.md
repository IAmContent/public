...
The _Calibrators_ class has convenient factory methods for creating _ServoSourceCalibrator_ objects, allowing them to be read from a JSON file. The default calibration file can be found at src/main/resources/servo/calibration/default.json. Alternatively, a _ServoSourceCalibrator_ named XXX can be obtained, which will be constructed based on the JSON file at src/main/resources/servo/calibration/XXX.json. Finally, the name of the default configuration can be changed from _default_ by setting the _iamcontent.servo.calibration_ System property.
